...
class WeaviateDB:
    ...
    def get_langchain_vectorstore(
        ...
        try:
            ...
            if multi_tenancy:
                ...
                return vectorstore.as_retriever(search_kwargs={'tenant': tentant_name, 'k': k, "return_uuids": True})
            else:
                return vectorstore.as_retriever(search_kwargs={'k': k, "return_uuids": True})
    ...
    def load_document_to_vectordb_llamaindex(
        ...
        pipeline = IngestionPipeline(
            transformations=[
                SentenceSplitter(chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVER_OVERLAP),
                AttatchTenant(),
                embedding,
            ],
            # docstore=docstore,
        ...
        # nodes = []
        node = pipeline.run(
            documents=documents, show_progress=True, num_workers=-1, tenant_name=tenant_name
        )
